...
- [x] **§12.4 Part 1 creado**: `summer_school_part1_lab.ipynb` (36 celdas, 20 code/16 md) vía script one-off `scripts/_build_part1.py`. Setup (credenciales + selección de backend + connectivity backend-aware) reusa infra de Part 2; usa los 3 ejemplos (`data/{documents,images,extracted}/`). Disclaimer "editar solo celdas 📝". Lógica de ejercicios+checks validada localmente contra datos reales (esqueletos→❌ sin crash, soluciones correctas→✅). **Sin validar en Colab**: `render_ontology` (necesita `dot`), `ingest_all`/`run_cypher`/`ask` (Neo4j/OpenAI) — espejan celdas/funciones ya probadas.
